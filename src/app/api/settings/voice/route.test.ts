...
vi.mock("@/lib/voice/access.server", () => ({
  getOwnerVoiceSettings: mocks.load, updateOwnerVoiceSettings: mocks.update,
  VoiceSettingsError: class extends Error { constructor(public code: string) { super(code); } },
}));
import { GET, PATCH } from "./route";
import { VoiceSettingsError } from "@/lib/voice/access.server";
const body = { mode: "voice", textFallbackEnabled: true, expectedRevision: 2 };
function request(value: unknown = body) { return new NextRequest("https://simplassist.com/api/settings/voice", { method: "PATCH", body: JSON.stringify(value) }); }
beforeEach(() => {
...
describe("owner voice settings routes", () => {
  ...
  it("returns only the current workspace's uncached projection", async () => {
    ...
    expect(await response.json()).toEqual({ voice: { visible: false } });
  });
  ...
  it("rejects invalid JSON", async () => {
    expect((await PATCH(new NextRequest("https://simplassist.com/api/settings/voice", { method: "PATCH", body: "{" }))).status).toBe(400);
  });
  it.each([["forbidden", 403], ["conflict", 409], ["unavailable", 503]] as const)("reports %s without automatically replaying the update", async (code, status) => {
    mocks.update.mockRejectedValue(new VoiceSettingsError(code)); expect((await PATCH(request())).status).toBe(status);
