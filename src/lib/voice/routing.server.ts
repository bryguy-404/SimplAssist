...
import { checkVoiceWorkerReady, type PilotRoutingDependencies } from "./routing";
...
export function pilotRoutingDependencies(): PilotRoutingDependencies {
  ...
  return {
    db: supabaseAdmin, telnyx, appUrl: process.env.NEXT_PUBLIC_APP_URL || "",
    workerUrl, streamSecret: process.env.VOICE_STREAM_SECRET || "",
    profile: process.env.VOICE_AUDIO_PROFILE === "pcmu8" ? "pcmu8" : "pcm16",
    workerReady: () => checkVoiceWorkerReady(workerUrl, token),
    sendFallback: async session => { await sendMissedCallSMS(session.caller_phone, session.business_id, session.call_session_id); },
  };
}
