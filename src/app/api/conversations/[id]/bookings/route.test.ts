...
vi.mock("@/lib/booking/review.server", () => ({ loadBookingReview: mocks.load }));
import { GET } from "./route";
const request = new NextRequest("https://simplassist.com/api/conversations/call/bookings");
beforeEach(() => { vi.clearAllMocks(); mocks.access.mockResolvedValue({ ok: true, access: { business: { id: "current-business" } } }); });
describe("owner booking review route", () => {
  ...
  it("uses the authorized business and returns a private uncached projection", async () => {
    ...
    expect(await response.json()).toEqual({ conversationId: "call", bookings: [] });
  });
