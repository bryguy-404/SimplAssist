...
**A passing local harness does not make the pilot callable.** Production migrations are applied and verified. The application is deployed with voice disabled and one approved tester is saved. OpenAI access, worker deployment/readiness, pilot activation and actual calls remain activation gates. Record actual deployment and acceptance results below; do not infer them from simulated audio or model availability.
...
- OpenAI setup (September 15): the key identified by Bryan in ADA-Calendar was securely copied to the voice worker with deployments skipped and its value verified without display. OpenAI model reads return HTTP 403: missing `api.model.read`. Enable that key permission in OpenAI, then recheck GPT-Live access. Bryan subsequently reported enabling Models Read. Fresh checks after that report still returned the same denial; a secret-safe comparison confirmed ADA-Calendar and the voice worker hold the same key. This denial does not establish whether the key has voice-session access. Activation remains blocked pending a successful provider access check.
...
- Application deployment (September 15): Railway deployment `e48d43f8-6340-4c65-b03e-35e3d0050118` is `SUCCESS`. Uploaded a clean Git archive of `0821aa1` (archive SHA-256 `562d0b8695368e599cd497a0df8358a4ea9b96970214ce8c09f8185b58ef7c6b`), excluding local environments, dependencies and build artifacts. Application rollout remains `false`; database pilot remains disabled. Both apex and www health checks returned 200, authenticated maintenance returned 200, and unauthenticated maintenance/admin mutations returned 404. Existing Telnyx callback and Claude model access passed read-only preflight. The app's GitHub source remains `main`; this was a one-off feature-branch upload, so a subsequent main deployment would replace this build. The scan worker was not redeployed or reconfigured.
- Approved testers (September 15): one caller number supplied explicitly by Bryan was saved through the authenticated production `/admin/voice` controls. Database read-back confirmed exactly that tester, settings revision 2, unchanged 200-minute allowance and voice disabled. Keep private caller numbers in pilot storage, not this document. The admin page was observed in the existing browser with the default limits and enablement blocked until service checks pass.
