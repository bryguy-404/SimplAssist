...
3. Verify the configured live Stripe Prices are the existing $10/$25/$45/$65 monthly products and one-time $25 fee, on the same Stripe account/mode as the webhook. Verify the configured Billing Portal permits payment-method management and cancellation but **does not permit subscription plan updates** that bypass this workflow.
